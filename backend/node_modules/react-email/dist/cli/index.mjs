...
const startDevServer = async (emailsDirRelativePath, staticBaseDirRelativePath, port, compatibilityClients) => {
	...
	devServer = http.createServer((req, res) => {
		...
		try {
			if (parsedUrl.pathname.includes("static/") && !parsedUrl.pathname.includes("_next/static/")) serveStaticFile(res, parsedUrl.pathname, staticBaseDirRelativePath);
			else if (!isNextReady) nextReadyPromise.then(() => nextHandleRequest?.(req, res));
			else nextHandleRequest?.(req, res);
		} catch (e) {
...
const renderingUtilitiesExporter = (emailTemplates) => ({
	...
	setup: (b) => {
		...
		b.onResolve({ filter: /^react-email-module-that-will-export-render$/ }, async (args) => {
			...
			if (result.errors.length > 0 && result.errors[0]) result.errors[0].text = "Failed trying to import `render` from either `@react-email/render` or `react-email` to be able to render your email template.\n Maybe you don't have either of them installed?";
			return result;
		});
	}
});
